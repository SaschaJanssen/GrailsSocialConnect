...
<div class="fieldcontain ${hasErrors(bean: keywordInstance, field: 'customer', 'error')} required">
	...
	<g:select id="customer" name="customer.id" from="${org.social.grails.customer.Customer.list()}" optionKey="id" optionValue="id" required="" value="${keywordInstance?.customer?.id}" class="many-to-one"/>
</div>
